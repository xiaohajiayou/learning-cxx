...
struct Tensor4D {
    ...
    Tensor4D &operator+=(Tensor4D const &others) {
        // TODO: 实现单向广播的加法
        for (auto i = 0u; i < 4; ++i) {
            ASSERT(shape[i] == others.shape[i] || others.shape[i] == 1, "Broadcast shape mismatch");
        }
            ...
            for (auto i1 = 0u; i1 < shape[1]; ++i1) {
                ...
                for (auto i2 = 0u; i2 < shape[2]; ++i2) {
                    ...
                    for (auto i3 = 0u; i3 < shape[3]; ++i3) {
                        ...
                        auto idx = ((i0 * shape[1] + i1) * shape[2] + i2) * shape[3] + i3;
                        auto oidx = ((o0 * others.shape[1] + o1) * others.shape[2] + o2) * others.shape[3] + o3;
